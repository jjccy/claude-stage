...
(function () {
    ...
    function setFace(fig, face) {
        const el = fig.el.querySelector('.face');
        if (el)
            el.textContent = face;
    }
    ...
    function updateHierarchyLines() {
        ...
        if (!claudeFig)
            return;
        let hasAgents = false;
        figures.forEach((_, id) => { if (id.startsWith('agent'))
            hasAgents = true; });
        if (!hasAgents)
            return;
        ...
        switch (event.type) {
            ...
            case 'tool_result': {
                ...
                if (success)
                    setFace(claude, '🤔');
                clearBubble(claude);
                setStatus(success ? 'Processing result...' : `Error in ${event.tool ?? 'tool'}`, success ? 'thinking' : 'error');
